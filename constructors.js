...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  Movie.prototype.changeRating= function(number){
    const averageRating = (this.rating + number)/2;
  ...
  User.prototype.removeSavedPost = function(id){
    const postToRemove = this.savedPosts.filter(post => post.id === id);
    const idOfPostToRemove = this.savedPosts.indexOf(postToRemove[0]);
    this.savedPosts.splice(idOfPostToRemove, 1);
  }
  ////////// PROBLEM 7 //////////
  ...
  User.prototype.changePostRating = function(id, rating){
    const postToUpdate = this.savedPosts.filter(post => post.id === id);
    postToUpdate[0].rating = rating;
  }